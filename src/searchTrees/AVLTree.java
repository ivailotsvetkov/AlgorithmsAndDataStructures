package searchTrees;
   ...
   public class AVLTree<T extends Comparable<T>> implements SearchTree<T>
   {
      ...
      private boolean checkHeight(Node<T> n)
      {
         if (n == null)
            return true;
         if (!checkHeight(n.getLeft()) || !checkHeight(n.getRight()))
            return false;
         return height(n) == 1 + Math.max(height(n.getLeft()),
               height(n.getRight()));
      }
      ...
      private Node<T> add(Node<T> n, T x)
      {
         if (x == null)
            throw new IllegalArgumentException(
                  "Attempt to insert null in the tree");
         if (n == null)
         {
            size++;
            return new Node<T>(x);
...
private T nthSmallest(Node<T> node, int n){
   return null;
      ...
      public Node<T> remove(Node<T> n, T x)
      {
         ...
         else
         {
            ...
            else
            {
               ...
               replacement.setHeight(1 + Math.max(
                     height(replacement.getLeft()),
                     height(replacement.getRight())));
               n = replacement;
            }
      ...
      private Node<T> rebalance(Node<T> n)
      {
         ...
         {
            if (height(n.getRight().getLeft()) <= height(n.getRight()
                  .getRight()))
               return rotateLeft(n);
            else
